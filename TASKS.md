...
- **Goal:** Crear la estructura de directorios del proyecto, el paquete Python `src`, el archivo de datos de ejemplo y las dependencias de desarrollo.
- **Files:**
  - `src/__init__.py`
  - `src/exceptions.py`
...
- **Acceptance:**
  - El paquete `src` es importable (`python -c "import src"`).
  - `data/customers.json` es JSON válido y contiene una lista de objetos con campos `id`, `name`, `email`.
  - Las excepciones `ValidationError` y `DataError` son importables desde `src.exceptions`.
- **Verification:**
  ```
  python -c "import src; print('OK')"
  python -c "import json; json.load(open('data/customers.json')); print('JSON OK')"
  python -c "from src.exceptions import ValidationError, DataError; print('Exceptions OK')"
  ```
...
- **Files:**
  - `src/models.py` — dataclass `Customer(id, name, email)`
  - `src/repository.py` — clase `CustomerRepository` con método `load() → list[Customer]`
- **Acceptance:**
...
- **Files:**
  - `src/normalizer.py` — funciones `normalize(text) → str`, `remove_accents(text) → str`
  - `src/service.py` — clase `SearchService` con método `search(query) → list[Customer]`
- **Acceptance:**
...
- **Files:**
  - `src/validator.py` — función `validate_query(query) → str` que lanza `ValidationError`
  - `src/api.py` — servidor HTTP en puerto 8000 con endpoint `GET /customers/search?q=`
  - `src/cli.py` — entry point CLI (`python -m src.cli <query>`)
  - `src/__main__.py` — permite `python -m src`
- **Acceptance:**
  ...
  - CLI `python -m src.cli "jose"` → imprime resultados, exit code 0 (NFR-02).
  - CLI `python -m src.cli ""` → imprime error en stderr, exit code 1 (FR-10).
- **Verification:**
  ```
  # Validador
  python -c "
...
- **Acceptance:**
  ...
  - Cobertura ≥ 90 % sobre `src/`: `pytest --cov=src --cov-fail-under=90` (NFR-03).
